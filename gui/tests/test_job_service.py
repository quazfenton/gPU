...
class TestJobService:
    ...
    def test_get_jobs_no_filters(self, job_service, mock_job_queue):
        ...
        # Verify
        assert len(result) == 3
        assert result[0]['job_id'] == "job-3"  # Most recent first (desc order)
    ...
    def test_get_jobs_with_template_filter(self, job_service, mock_job_queue):
        ...
        # Verify
        assert len(result) == 2
        assert all(job['template'] == 'template-a' for job in result)
    ...
    def test_get_jobs_with_sorting(self, job_service, mock_job_queue):
        ...
        # Verify
        assert len(result) == 3
        assert result[0]['job_id'] == "job-3"  # 1 min
        assert result[1]['job_id'] == "job-1"  # 2 min
        assert result[2]['job_id'] == "job-2"  # 5 min
    ...
    def test_get_job_logs_with_metadata(self, job_service, mock_job_queue):
        """Test retrieving job logs from metadata."""
        # Setup
        job = Job(
            id="job-123",
            user_id="user-1",
        ...
        # Execute
        logs = job_service.get_job_logs("job-123")
