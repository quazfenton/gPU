            ...
            try:
                ...
                if code_file:
                    ...
                    if nb_path.exists() and nb_path.stat().st_size > 0:
                        ...
                        logger.warning("Pulled notebook is missing or empty, retrying via HTTP API...")
                        return kernel_pull_alternative(ref, dest)
            except Exception as me:
                logger.warning(f"Could not read kernel-metadata.json ({me}); attempting fallback detection")
...
def _generate_title_from_filename(code_file):
    ...
    return (code_file.replace('.ipynb', '')
                     .replace('.py', '')
                     .replace('_', ' ')
                     .replace('-', ' ')
                     .title())
...
def determine_input_type(input_str):
    ...
    if input_str.startswith(('http://', 'https://')):
        ...
        elif any(host in input_str for host in ['github.com', 'gist.github.com', 'gitlab.com', 'bitbucket.org', 'drive.google.com', 'docs.google.com']):
            return "remote_notebook"
        return "other_url"

    # Check for Kaggle reference format (username/kernel-name)
    if '/' in input_str and not input_str.startswith(('.', '/')) and len(input_str.split('/')) == 2:
...
def _maybe_github_blob_to_raw(url: str) -> str:
    ...
    try:
        if 'github.com' in url and '/blob/' in url:
            ...
            owner, repo = owner_repo.split('/') if '/' in owner_repo else owner_repo.split('/')
            # Rebuild raw URL
...
def download_google_drive_file(url):
    ...
    try:
        ...
        temp_file = tempfile.NamedTemporaryFile(mode='w', suffix='.ipynb', delete=False, encoding='utf-8')
...
def download_remote_notebook(url):
    ...
    try:
        ...
        if response is None or response.status_code != 200:
            logger.error(f"Failed to download from {url} (normalized to {normalized_url})")
            return None
        ...
        if '.ipynb' not in url and not url.endswith('.ipynb'):
            ...
            try:
                content_str = response.text if isinstance(response.text, str) else response.text.encode('utf-8').decode('utf-8')
                json.loads(content_str[:500])  # Check first 500 chars for JSON structure
        ...
        logger.info(f"Downloaded notebook from {url} (normalized to {normalized_url}) to {temp_file.name}")
        return temp_file.name
    except Exception as e:
...
def interactive_kernel_selection(use_popular=False):
    ...
    try:
        ...
        while True:
            choice = input(f"\nChoose kernel number (1-{min(len(kernels), 20)}) or 'p' for popular, 'q' to quit: ").strip()
            if choice.lower() == 'q':
                return None
            ...
            try:
                ...
                print("Invalid selection, please choose a number between 1 and", min(len(kernels), 20))
            except ValueError:
...
def cmd_run(args):
    ...
    deploy_opts = _collect_gcp_opts_from_args(args) if getattr(args, 'deploy', False) else None
    ...
    if hasattr(args, 'input') and args.input:
        ...
        if input_type == "kaggle_url":
            ...
            source_dir_for_deploy = Path(args.input).parent if Path(args.input).is_file() else Path(args.input)
        elif input_type == "remote_notebook":
            if temp_file := download_remote_notebook(args.input):
                try:
    ...
    try:
        if getattr(args, 'deploy', False):
            deploy_to_gcloud(source_dir_for_deploy or Path(getattr(args, 'dest', '.')), opts=deploy_opts)
    except Exception as e:
        ...
        "cells": [
            {
                "cell_type": "markdown",
                "metadata": {},
                "source": ["# New Kaggle Notebook\n\nCreated with kaggle.py CLI tool"]
            },
            {
                "cell_type": "code",
...
'''
    lambda_py.write_text(template.format(notebook_code=notebook_script), encoding='utf-8')
    ...
    try:
        ...
        if ok and url:
            try:
                ...
                register_endpoint(name_for_registry or fname, url, provider='gcp-functions', metadata={'project': project, 'region': region})
            except Exception as reg_e:
                logger.warning(f"Could not register endpoint locally: {reg_e}")
        elif ok and not url:
            logger.warning("Deployment reported success, but no URL was parsed from output.")
        return ok
...
def _detect_model_requirements(project_root: Path) -> list:
    ...
    try:
        ...
        if re.search(r"\bimport\s+kagglehub\b|from\s+kagglehub\s+import\b", text):
            seen('kagglehub')
        if re.search(r"\bimport\s+huggingface_hub\b|from\s+huggingface_hub\s+import\b", text):
...
def deploy_with_gcloud(deploy_dir: Path, opts: dict | None = None):
    ...
    try:
        ...
        project = opts.get('project') or os.environ.get('GOOGLE_CLOUD_PROJECT') or os.environ.get('PROJECT_ID')
        region = opts.get('region') or os.environ.get('REGION', 'us-central1')
        function_name = opts.get('function_name') or f"kaggle-notebook-{int(time.time())}"
        ...
        result = subprocess.run(cmd, check=True, capture_output=True, text=True, cwd=str(deploy_dir))
...
def _collect_gcp_opts_from_args(args) -> dict:
    return {
        'project': getattr(args, 'gcp_project', None) or os.environ.get('GOOGLE_CLOUD_PROJECT') or os.environ.get('PROJECT_ID'),
        'region': getattr(args, 'region', None) or os.environ.get('REGION', 'us-central1'),
...
def cmd_package_aws(args):
    ...
    try:
        ...
        print("aws lambda create-function --function-name my-func --runtime python3.11 --zip-file fileb://aws-lambda.zip --handler lambda_function.handler --role arn:aws:iam::<ACCOUNT_ID>:role/<ROLE_NAME>")
    except Exception as e:
...
def cmd_serve_local(args):
    ...
    try:
        ...
        if getattr(args, 'run', False):
            try:
                cmd = [sys.executable, '-m', 'functions_framework', '--target', 'predict_handler', '--port', str(port)]
            ...
            print(f"(cd {deploy_dir} && {sys.executable} -m functions_framework --target predict_handler --port {port})")
    except Exception as e:
...
def validate_setup():
    ...
    try:
        ...
        issues.append('gcloud CLI not found (optional for deployment). Install Google Cloud SDK')
...
def cmd_batch(args):
    ...
    try:
        ...
        with open(args.input_file, 'r', encoding='utf-8') as f:
            notebook_list = [line.strip() for line in f if line.strip() and not line.startswith('#')]
            ...
            try:
                if args.operation == 'download':
                    ...
                    class TempDeployArgs:
                        def __init__(self, path, gcp_project, region, function_name, memory, timeout, save_name, public):
                            self.path = str(path)
                    ...
                    if 'kaggle.com' in str(notebook_ref) or '/' in str(notebook_ref):
                        ...
                        pull_args = type('TempArgs', (), {'kernel': notebook_ref, 'dest': str(temp_dir)})()
                    ...
                    class TempRunArgs:
                        def __init__(self, input_val, dest, deploy):
                            ...
                            self.dest = dest
                            self.deploy = deploy
                    run_args = TempRunArgs(input_val=notebook_ref, dest=args.output_dir, deploy=False)
                    cmd_run(run_args)
                success_count += 1
            except Exception as e:
                logger.error(f'Failed to {args.operation} {notebook_ref}: {e}')
        
        logger.info(f'Batch processing completed: {success_count}/{len(notebook_list)} successful')
    except Exception as e:
...
def cmd_preprocess(args):
    ...
    try:
        if input_path.is_file() and input_path.suffix == '.ipynb':
            ...
            logger.error(f'Input path must be a notebook file or directory: {input_path}')
            return
        ...
        logger.info(f'Preprocessing completed: {processed_count}/{len(notebooks)} notebooks processed')
    except Exception as e:
        logger.error(f'Preprocessing failed: {e}')

def preprocess_notebook(notebook_path, clean_metadata=False, remove_outputs=False, scan_security=False, optimize_imports=False):
        ...
        if cell.get('cell_type') == 'code':
            ...
            for pattern in credential_patterns:
                if re.search(pattern, source, re.IGNORECASE):
                    issues_found.append(f'Potential credential found in cell: {source[:100]}...')
    
    if issues_found:
        logger.warning(f'Security scan found {len(issues_found)} potential issues in {notebook_path}:')
...
Examples:
  ...
  %(prog)s deploy ./notebook-directory             # Deploy existing notebook
    ...
    pull_parser.add_argument('kernel', help='Kernel reference (user/kernel) or Kaggle URL')
    pull_parser.add_argument('--dest', default='.', help='Destination directory (default: current)')
    ...
    create_parser.add_argument('input', nargs='?', help='Local file, remote URL, or empty for blank notebook')
    create_parser.set_defaults(func=cmd_create)

    # Run command
    run_parser = subparsers.add_parser('run', help='Run/execute kernel')
    run_parser.add_argument('input', nargs='?', help='Kernel ref, URL, local file, or empty for interactive selection')
    run_parser.add_argument('--dest', default='.', help='Destination directory for pulled files')
    run_parser.add_argument('--deploy', action='store_true', help='Deploy to Google Cloud Functions after running')
    # GCP deploy options for run --deploy
    run_parser.add_argument('--gcp-project', help='GCP Project ID')
    run_parser.add_argument('--region', default=None, help='GCP region (default: env REGION or us-central1)')
    run_parser.add_argument('--function-name', dest='function_name', help='Cloud Function name (default autogenerated)')
    run_parser.add_argument('--memory', default='512MB', help='Function memory (default: 512MB)')
    run_parser.add_argument('--timeout', default='540s', help='Function timeout (default: 540s)')
    run_parser.add_argument('--save-name', dest='save_name', help='Save endpoint under this name for later use')
    run_parser.add_argument('--private', dest='public', action='store_false', help='Require authentication; default is public')
    run_parser.set_defaults(func=cmd_run, public=True)

    # Deploy command
    deploy_parser = subparsers.add_parser('deploy', help='Deploy existing notebook directory or file')
    deploy_parser.add_argument('path', help='Path to notebook file or directory containing notebook')
    deploy_parser.add_argument('--gcp-project', help='GCP Project ID')
    deploy_parser.add_argument('--region', default=None, help='GCP region (default: env REGION or us-central1)')
    deploy_parser.add_argument('--function-name', dest='function_name', help='Cloud Function name (default autogenerated)')
    deploy_parser.add_argument('--memory', default='512MB', help='Function memory (default: 512MB)')
    deploy_parser.add_argument('--timeout', default='540s', help='Function timeout (default: 540s)')
    deploy_parser.add_argument('--save-name', dest='save_name', help='Save endpoint under this name for later use')
    deploy_parser.add_argument('--private', dest='public', action='store_false', help='Require authentication; default is public')
    deploy_parser.set_defaults(func=cmd_deploy, public=True)

    # AWS packaging command
    aws_pkg_parser = subparsers.add_parser('package-aws', help='Package notebook as AWS Lambda zip (no deploy)')
    aws_pkg_parser.add_argument('path', help='Path to notebook file or directory containing notebook')
    aws_pkg_parser.set_defaults(func=cmd_package_aws)

    # Local serve command (Jupyter/local dev support)
    serve_parser = subparsers.add_parser('serve-local', help='Package notebook and (optionally) run a local HTTP server')
    serve_parser.add_argument('path', help='Path to notebook file or directory containing notebook')
    serve_parser.add_argument('--port', default='8080', help='Port to run the local server (default: 8080)')
    serve_parser.add_argument('--run', action='store_true', help='Actually start the server after packaging')
    ...
    ep_call = subparsers.add_parser('call', help='Call an endpoint by name or URL with JSON')
    ...
    ep_call.add_argument('--json', dest='json_inline', help='Inline JSON payload as string')
    ep_call.add_argument('--json-file', dest='json_file', help='Path to JSON file payload')
    ep_call.set_defaults(func=_call_func)
    # Doctor command (runs validation)
    doc_parser = subparsers.add_parser('doctor', help='Check environment (Kaggle CLI, gcloud, credentials)')
    ...
    batch_parser.add_argument('input_file', help='File containing list of notebooks to process (one per line)')
    ...
    preprocess_parser = subparsers.add_parser('preprocess', help='Preprocess notebook files')
    preprocess_parser.add_argument('input_path', help='Path to notebook file or directory')
    preprocess_parser.add_argument('--clean-metadata', action='store_true', 
                                   help='Remove unnecessary metadata from notebook')
    preprocess_parser.add_argument('--remove-outputs', action='store_true', 
                                   help='Remove output cells from notebook')
    preprocess_parser.add_argument('--scan-security', action='store_true', 
                                   help='Scan for hardcoded credentials and security issues')
    preprocess_parser.add_argument('--optimize-imports', action='store_true', 
                                   help='Analyze and optimize import statements')
    preprocess_parser.add_argument('--output', help='Output path for processed notebook (default: overwrite input)')
    ...
    if not args.command:
        parser.print_help()
        print("\nFor detailed help on a command, use: python kaggle.py <command> --help")
        return
