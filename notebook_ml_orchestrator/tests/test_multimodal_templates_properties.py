"""
Property-based tests for Multimodal templates.

This module implements property-based tests using Hypothesis to verify
that Multimodal templates conform to the design specifications for input/output types,
...
def multimodal_template_strategy():
    """Strategy for generating Multimodal template instances."""
...
class TestMultimodalTemplateMultipleInputTypes:
    """
    **Property 4: Multimodal template multiple input types**
    
    For any template in the Multimodal category, the template SHALL declare 
    at least two distinct modality types across inputs and outputs from the set 
    {image, text, video, audio}. This validates that the template bridges multiple
    modalities (e.g., image-to-text, text-to-image, image+text-to-text).
    Configuration parameters (number, etc.) don't count toward this requirement.
    
    **Validates: Requirements 4.4**
    """
    
    @settings(max_examples=100)
    @given(multimodal_template_strategy())
    def test_multimodal_template_has_multiple_modality_types(self, template):
        """Multimodal templates must have at least two distinct modality types across inputs and outputs."""
        # Verify template is in Multimodal category
        assert template.category == "Multimodal", f"Template {template.name} is not in Multimodal category"
        
        # Define modality types (not configuration types)
        modality_types = {"image", "text", "video", "audio"}
        
        # Collect distinct modality types from inputs and outputs
        modalities = set()
        for input_field in template.inputs:
            if input_field.type in modality_types:
                modalities.add(input_field.type)
        for output_field in template.outputs:
            if output_field.type in modality_types:
                modalities.add(output_field.type)
        
        assert len(modalities) >= 2, (
            f"Multimodal template '{template.name}' must have at least two distinct modality types "
            f"across inputs and outputs. Found modality types: {modalities}"
        )
    
    @settings(max_examples=100)
    @given(multimodal_template_strategy())
    def test_multimodal_template_has_at_least_one_input(self, template):
        """Multimodal templates must have at least one input field."""
        assert len(template.inputs) > 0, (
            f"Multimodal template '{template.name}' has no input fields"
        )
    
    @settings(max_examples=100)
    @given(multimodal_template_strategy())
    def test_multimodal_template_has_at_least_one_output(self, template):
        """Multimodal templates must have at least one output field."""
        assert len(template.outputs) > 0, (
            f"Multimodal template '{template.name}' has no output fields"
        )
