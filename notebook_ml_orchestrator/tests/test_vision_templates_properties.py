"""
Property-based tests for Vision templates.

This module implements property-based tests using Hypothesis to verify
that Vision templates conform to the design specifications for input/output types,
...
def vision_template_strategy():
    """Strategy for generating Vision template instances."""
    return st.sampled_from([
        ObjectDetectionTemplate(),
        ImageSegmentationTemplate(),
        VideoProcessingTemplate()
    ])


def valid_vision_input_types():
    """Valid input types for Vision templates."""
    return st.sampled_from(["image", "video"])


def valid_vision_output_types():
    """Valid output types for Vision templates."""
    return st.sampled_from(["image", "video", "json"])
...
class TestVisionTemplateIOTypes:
    """
    **Property 2: Vision template I/O types**
    
    For any template in the Vision category, the template SHALL have at least one 
    primary data input of type "image" or "video", and all output types SHALL be 
    either "image", "video", or "json". Configuration parameters (number, text, etc.) 
    ...
    **Validates: Requirements 2.4, 2.5**
    ...
    @given(vision_template_strategy())
    def test_vision_template_has_primary_image_or_video_input(self, template):
        """Vision templates must have at least one 'image' or 'video' input (primary data)."""
        # Verify template is in Vision category
        assert template.category == "Vision", f"Template {template.name} is not in Vision category"
        
        # Check that at least one input is image or video (the primary data input)
        primary_data_types = {"image", "video"}
        ...
        assert has_primary_input, (
            f"Vision template '{template.name}' must have at least one input of type 'image' or 'video'. "
            f"Found input types: {[f.type for f in template.inputs]}"
        )
    
    @settings(max_examples=100)
    @given(vision_template_strategy())
    def test_vision_template_output_types_are_valid(self, template):
        """All Vision template outputs must be 'image', 'video', or 'json' types."""
        # Verify template is in Vision category
        assert template.category == "Vision", f"Template {template.name} is not in Vision category"
        
        # Check all output types - vision templates can output image, video, or json
        valid_output_types = {"image", "video", "json"}
        for output_field in template.outputs:
            assert output_field.type in valid_output_types, (
                f"Vision template '{template.name}' has invalid output type '{output_field.type}'. "
                f"Expected one of {valid_output_types}"
            )
    
    @settings(max_examples=100)
    @given(vision_template_strategy())
    def test_vision_template_has_at_least_one_input(self, template):
        """Vision templates must have at least one input field."""
        assert len(template.inputs) > 0, (
            f"Vision template '{template.name}' has no input fields"
        )
    
    @settings(max_examples=100)
    @given(vision_template_strategy())
    def test_vision_template_has_at_least_one_output(self, template):
        """Vision templates must have at least one output field."""
        assert len(template.outputs) > 0, (
            f"Vision template '{template.name}' has no output fields"
        )
